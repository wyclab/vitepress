(function(exports, _vueuse_shared, async_validator, vue, axios, change_case, universal_cookie, _vueuse_core, drauu, focus_trap, fuse_js, idb_keyval, jwt_decode, nprogress, qrcode, sortablejs) {
	...
	*/
	function useAsyncValidator(value, rules, options = {}) {
		...
		const valueRef = (0, _vueuse_shared.toRef)(value);
		...
		function waitUntilFinished() {
			return new Promise((resolve, reject) => {
				(0, _vueuse_shared.until)(isFinished).toBe(true).then(() => resolve(shell)).catch((error) => reject(error));
			});
		}
		return {
			...shell,
 ...
 /**
	...
	*/
	...
	function useFuse(search, data, options) {
		...
		const fuse = (0, vue.shallowRef)(createFuse());
		(0, vue.watch)(() => {
			var _toValue3;
	...
	*/
	function useNProgress(currentProgress = null, options) {
		const progress = (0, _vueuse_shared.toRef)(currentProgress);
		...
		(0, vue.watchEffect)(() => {
			if (typeof progress.value === "number" && _vueuse_shared.isClient) setProgress.call(nprogress.default, progress.value);
		});
		(0, _vueuse_shared.tryOnScopeDispose)(nprogress.default.remove);
		return {
			isLoading,
			progress,
	...
	*/
	function useSortable(el, list, options = {}) {
		let sortable;
		const { document = _vueuse_core.defaultDocument, watchElement = false, ...resetOptions } = options;
		...
		const start = () => {
			const target = typeof el === "string" ? document === null || document === void 0 ? void 0 : document.querySelector(el) : (0, _vueuse_core.unrefElement)(el);
		...
		let stopWatch;
		if (watchElement && typeof el !== "string") stopWatch = (0, vue.watch)(() => (0, _vueuse_core.unrefElement)(el), (newElement) => {
			cleanup();
		...
		else (0, _vueuse_core.tryOnMounted)(start);
		const stop = () => {
			cleanup();
		};
